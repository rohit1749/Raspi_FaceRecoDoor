...
# Using LBPH(Local Binary Patterns Histograms) recognizer MODEL USED FOR TRAINING
...
def getImagesAndLabels(path):
    ...
    for file_name in os.listdir(path):
        if file_name.endswith(".jpg"):
            ...
            img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)#load an image from specified file in grayscale mode

            faces = face_detector.detectMultiScale(img)#return the coordinate of bounding box

            for (x, y, w, h) in faces:
                faceSamples.append(img[y:y+h, x:x+w])#to crop the image
                ids.append(id) #id denotes the user no.
...
faces, ids = getImagesAndLabels(path) #returned values get stored in faces and ids
